<?php get_header(); ?>
			<div class="wrapper-content">
				
					<div class="content-text">
						<h1>Want anything to be easy with <strong>LaslesVPN.</strong></h1>
						<p>Provide a network for all your needs with ease and fun using LaslesVPN discover interesting features from us.</p>
					</div>
					<div class="content-img">
							<img class="logo" src="./img/ig.png" >
					</div>
			</div>
			<div class="wrapper-content1">
				<div class="wrapper-item">
					<img class="icon-user" src="./img/user.jpg">
					<div class="content-user">
						<h2><strong>90+</strong></h2>
						<span>User</span>
					</div>
				</div>
				<div class="wrapper-item">
					<img class="icon-user" src="./img/user.jpg">
					<div class="content-user">
						<h2><strong>90+</strong></h2>
						<span>User</span>
					</div>
				</div>
				<div class="wrapper-item">
					<img class="icon-user" src="./img/user.jpg">
					<div class="content-user">
						<h2><strong>90+</strong></h2>
						<span>User</span>
					</div>
				</div>
			</div>
			<div class="wrapper-content2">
				<div class="wrap-choose">
					<img class="imgbox" src="./img/box.jpg">
					<div class="content-box">
							<h2 class="title">Free Plan</h2>
							<ul class="content-textchoose">
								<li>
								<img src="./img/stick.png">
								<span>Unlimited Bandwitch</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Encrypted Connection</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>No Traffic Logs</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Works on All Devices</span>
								</li>
							</ul>
					</div> 
					<div class="btn-price">
						<p>FREE</p>
						<a href="#">Select</a>
					</div>
				</div>
				<div class="wrap-choose">
					<img class="imgbox" src="./img/box.jpg">
					<div class="content-box">
							<h2 class="title">Free Plan</h2>
							<ul class="content-textchoose">
								<li>
								<img src="./img/stick.png">
								<span>Unlimited Bandwitch</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Encrypted Connection</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>No Traffic Logs</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Works on All Devices</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Connect Anyware</span>
								</li>
							</ul>
					</div> 
					<div class="btn-price">
						<p>FREE</p>
						<a href="#">Select</a>
					</div>
				</div>
				<div class="wrap-choose">
					<img class="imgbox" src="./img/box.jpg">
					<div class="content-box">
							<h2 class="title">Free Plan</h2>
						
							<ul class="content-textchoose">
								<li>
								<img src="./img/stick.png">
								<span>Unlimited Bandwitch</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Encrypted Connection</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>No Traffic Logs</span>
								</li>
								<li>
								<img src="./img/stick.png">
								<span>Works on All Devices</span>
								</li>
							</ul>
						
					</div> 
					<div class="btn-price">
						<p>FREE</p>
						<a href="#">Select</a>
					</div>
				</div>

			</div>
			<div class="wrapper-content3">
				<div class="bentrai">
					<img src="./img/09.jpg" />
				</div>
                <div class="benphai">
				<div class="customer">
					<div class="custom-top">
						<div class="info-user">
							<img src="./img/user1.jpg">
							<div class="info-titleuser">
								<strong>Viezh Robert</strong>
								<p>Shanxi, China</p>
							</div>
						</div>
						<div class="info-star">
							<span>4.5</span>
							<img src="./img/star.jpg">
						</div>

					</div>
					<div class="custome-bottom">
						<p>“Wow... I am very happy to use this VPN, it turned out to be more than my expectations and so far there have been no problems. LaslesVPN always the best”.</p>
					</div>
				</div>
				<div class="customer">
					<div class="custom-top">
						<div class="info-user">
							<img src="./img/user1.jpg">
							<div class="info-titleuser">
								<strong>Viezh Robert</strong>
								<p>Shanxi, China</p>
							</div>
						</div>
						<div class="info-star">

						</div>

					</div>
					<div class="custome-bottom">
						<p>“Wow... I am very happy to use this VPN, it turned out to be more than my expectations and so far there have been no problems. LaslesVPN always the best”.</p>
					</div>
				</div>
				<div class="customer">
					<div class="custom-top">
						<div class="info-user">
							<img src="./img/user1.jpg">
							<div class="info-titleuser">
								<strong>Viezh Robert</strong>
								<p>Shanxi, China</p>
							</div>
						</div>
						<div class="info-star">

						</div>

					</div>
					<div class="custome-bottom">
						<p>“Wow... I am very happy to use this VPN, it turned out to be more than my expectations and so far there have been no problems. LaslesVPN always the best”.</p>
					</div>
				</div>
				<div class="customer">
					<div class="custom-top">
						<div class="info-user">
							<img src="./img/user1.jpg">
							<div class="info-titleuser">
								<strong>Viezh Robert</strong>
								<p>Shanxi, China</p>
							</div>
						</div>
						<div class="info-star">

						</div>

					</div>
					<div class="custome-bottom">
						<p>“Wow... I am very happy to use this VPN, it turned out to be more than my expectations and so far there have been no problems. LaslesVPN always the best”.</p>
					</div>
				</div>
			</div>
			</div>
			<div class="wrap-content4">
				<div class="wrap-listslider">
					<div class="img">
						<img src="./img/01.jpg"/>
					</div>
					<div class="img">
						<img src="./img/02.jpg"/>
					</div>
					<div class="img">
						<img src="./img/03.jpg"/>
					</div>
					<div class="img">
						<img src="./img/04.jpg"/>
					</div>
					<div class="img">
						<img src="./img/05.jpg"/>
					</div>
					<div class="img">
						<img src="./img/06.jpg"/>
					</div>
					<div class="img">
						<img src="./img/09.jpg"/>
					</div>
				</div>
				
			</div>
			<div class="wrap-popupslider an">
					<div class="wrap-img-box">
						<a class="prev">prev</a>
						<img src="./img/01.jpg"/>
						<a class="next">next</a>
						<a class="close">close</a>
					</div>
				</div>
				<div class="wrapper-content7">
					<p>sola1</p>
					<p>sola2</p>
					<p>sola3</p>
					<p>sola4</p>
					<p>sola5</p>
					<p>sola6</p>
					<p>sola7</p>

				</div>
		</div>
		<?php get_footer(); ?>			